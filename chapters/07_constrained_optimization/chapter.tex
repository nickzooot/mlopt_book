...
\begin{figure}[ht]
    ...
    \caption{kkt condition}
    \label{fig:kkt-condition}
\end{figure}
...
\begin{itemize}
  ...
  $\mathbf{d}$. As a consequence,  every direction is feasible, whereas 
  the actual feasible set may admit only a
  much smaller set of directions (in our case\footnote{see left part of \cref{fig:kkt-condition}} this set is still $T_F(\mathbf{x}^\star)$ because this geometric
  object doesn't depend on whether $\nabla h (\mathbf{x}^\star)$ is 0 or not). This is the first sign that a constraint
...
\begin{enumerate}
  ...
  The family
  \[
    \left\{\nabla g_i(\mathbf{x}^\star),\nabla h_j(\mathbf{x}^\star): i,j \in Active(\mathbf{x}^\star)\right\}
  \]
