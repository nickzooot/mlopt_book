...
$\kappa$ increases (see \cref{fig:ch5-linear-cg-behaviour}).
\end{example}

\begin{figure}[ht]
  \centering
  \incfig{linear-cg-behaviour-embed}
  \caption{A conditioning experiment for gradient descent and conjugate
  gradient. Left: on this random diagonal ensemble, the main visible effect for
  gradient descent is the roughly linear growth with the condition number; in
  this test the curves for larger $n$ happen to lie lower, so the point of the
  panel is the $\kappa$-dependence rather than a monotone dependence on $n$.
  Right: for CG the growth is much slower and follows the familiar
  $O(\sqrt{\kappa})$ pattern until the finite-termination ceiling at $n$
  becomes visible, most clearly for $n=10$. The dashed lines show the
  corresponding $\kappa$- and $\sqrt{\kappa}$-type approximations for each
  solid curve.}
...
\underbrace{\mathbf{S}^{-\top}\mathbf{A}\mathbf{S}^{-1}}_{=:\tilde{\mathbf{A}} \succ 0}
  \tilde{\mathbf{x}}
  =
...
The clean way to derive the method is to apply the ordinary linear CG scheme
from \cref{sec:ch5-linear-cg} to the transformed system and then map every
quantity back to the original variables.

\begin{claim}[PCG as linear CG in transformed coordinates]
\label{clm:ch5-pcg-derivation}
Let
\[
  \tilde{\mathbf{g}}_k:=\tilde{\mathbf{A}}\tilde{\mathbf{x}}_k-\tilde{\mathbf{b}},
  \qquad
  \tilde{\mathbf{d}}_0=-\tilde{\mathbf{g}}_0,
  \qquad
  \tilde{\mathbf{d}}_{k+1}
  =-\tilde{\mathbf{g}}_{k+1}+\tilde{\beta}_k \tilde{\mathbf{d}}_k
\]
be the ordinary CG recurrence for the transformed system
\[
  \tilde{\mathbf{A}}\tilde{\mathbf{x}}=\tilde{\mathbf{b}}.
\]
Define
\[
  \mathbf{x}_k:=\mathbf{S}^{-1}\tilde{\mathbf{x}}_k.
\]
Since the transformed iterate update is
\[
  \tilde{\mathbf{x}}_{k+1}
  =\tilde{\mathbf{x}}_k+\tilde{\alpha}_k \tilde{\mathbf{d}}_k,
\]
the corresponding search directions in the original variables must be pulled
back as
\[
  \mathbf{d}_k:=\mathbf{S}^{-1}\tilde{\mathbf{d}}_k,
\]
because then
\[
  \mathbf{x}_{k+1}
  =\mathbf{S}^{-1}\tilde{\mathbf{x}}_{k+1}
  =\mathbf{S}^{-1}(\tilde{\mathbf{x}}_k+\tilde{\alpha}_k \tilde{\mathbf{d}}_k)
  =\mathbf{x}_k+\tilde{\alpha}_k \mathbf{d}_k.
\]
Finally define the residuals of the original system by
\[
...
Then
\[
  \tilde{\mathbf{g}}_k=\mathbf{S}^{-\top}\mathbf{g}_k,
  \qquad
  \mathbf{d}_0=-\mathbf{S}^{-1}\mathbf{S}^{-\top}\mathbf{g}_0,
  \qquad
  \mathbf{x}_{k+1}=\mathbf{x}_k+\alpha_k \mathbf{d}_k,
  \qquad
  \mathbf{g}_{k+1}=\mathbf{g}_k+\alpha_k \mathbf{A}\mathbf{d}_k,
...
\[
  \mathbf{d}_{k+1}
  =-\mathbf{S}^{-1}\mathbf{S}^{-\top}\mathbf{g}_{k+1}+\beta_k \mathbf{d}_k.
\]
Moreover, the transformed CG coefficients become
\[
  \alpha_k=\tilde{\alpha}_k
  =\frac{\mathbf{g}_k^{\top}\mathbf{S}^{-1}\mathbf{S}^{-\top}\mathbf{g}_k}
  {\mathbf{d}_k^{\top}\mathbf{A}\mathbf{d}_k},
  \qquad
  \beta_k=\tilde{\beta}_k
  =\frac{\mathbf{g}_{k+1}^{\top}\mathbf{S}^{-1}\mathbf{S}^{-\top}\mathbf{g}_{k+1}}
  {\mathbf{g}_k^{\top}\mathbf{S}^{-1}\mathbf{S}^{-\top}\mathbf{g}_k}.
\]
\end{claim}
\begin{proof}
The update formula for $\mathbf{x}_{k+1}$ was already built into the definition
of $\mathbf{d}_k$, so it remains to identify how the gradients transform and
then substitute that relation into the CG recurrence.

\noindent First,
\[
  \tilde{\mathbf{g}}_k
  =\tilde{\mathbf{A}}\tilde{\mathbf{x}}_k-\tilde{\mathbf{b}}
  =\mathbf{S}^{-\top}\mathbf{A}\mathbf{S}^{-1}\tilde{\mathbf{x}}_k
   -\mathbf{S}^{-\top}\mathbf{b}
  =\mathbf{S}^{-\top}(\mathbf{A}\mathbf{x}_k-\mathbf{b})
  =\mathbf{S}^{-\top}\mathbf{g}_k.
\]
Therefore
\[
  \mathbf{d}_0
  =\mathbf{S}^{-1}\tilde{\mathbf{d}}_0
  =-\mathbf{S}^{-1}\tilde{\mathbf{g}}_0
  =-\mathbf{S}^{-1}\mathbf{S}^{-\top}\mathbf{g}_0.
\]
Substituting this into the definition of $\mathbf{g}_{k+1}$ gives
\[
  \mathbf{g}_{k+1}
  =\mathbf{A}\mathbf{x}_{k+1}-\mathbf{b}
  =\mathbf{A}(\mathbf{x}_k+\tilde{\alpha}_k \mathbf{d}_k)-\mathbf{b}
  =\mathbf{g}_k+\tilde{\alpha}_k \mathbf{A}\mathbf{d}_k.
\]
Finally,
\[
  \mathbf{d}_{k+1}
  =\mathbf{S}^{-1}\tilde{\mathbf{d}}_{k+1}
  =-\mathbf{S}^{-1}\tilde{\mathbf{g}}_{k+1}+\tilde{\beta}_k \mathbf{S}^{-1}\tilde{\mathbf{d}}_k
  =-\mathbf{S}^{-1}\mathbf{S}^{-\top}\mathbf{g}_{k+1}+\tilde{\beta}_k \mathbf{d}_k.
\]
\noindent For the coefficients, the transformed CG formulas give
\[
  \tilde{\alpha}_k
  =\frac{\tilde{\mathbf{g}}_k^{\top}\tilde{\mathbf{g}}_k}
  {\tilde{\mathbf{d}}_k^{\top}\tilde{\mathbf{A}}\tilde{\mathbf{d}}_k}.
\]
The numerator satisfies
\[
  \tilde{\mathbf{g}}_k^{\top}\tilde{\mathbf{g}}_k
  =\mathbf{g}_k^{\top}\mathbf{S}^{-1}\mathbf{S}^{-\top}\mathbf{g}_k,
\]
while the denominator becomes
\[
  \tilde{\mathbf{d}}_k^{\top}\tilde{\mathbf{A}}\tilde{\mathbf{d}}_k
  =(\mathbf{S}\mathbf{d}_k)^{\top}
   (\mathbf{S}^{-\top}\mathbf{A}\mathbf{S}^{-1})
   (\mathbf{S}\mathbf{d}_k)
  =\mathbf{d}_k^{\top}\mathbf{A}\mathbf{d}_k.
\]
Hence
\[
...
\[
  \tilde{\beta}_k
  =\frac{\tilde{\mathbf{g}}_{k+1}^{\top}\tilde{\mathbf{g}}_{k+1}}
  {\tilde{\mathbf{g}}_k^{\top}\tilde{\mathbf{g}}_k}
  =\frac{\mathbf{g}_{k+1}^{\top}\mathbf{S}^{-1}\mathbf{S}^{-\top}\mathbf{g}_{k+1}}
  {\mathbf{g}_k^{\top}\mathbf{S}^{-1}\mathbf{S}^{-\top}\mathbf{g}_k}.
\]
Renaming $\tilde{\alpha}_k,\tilde{\beta}_k$ as $\alpha_k,\beta_k$ in the
original coordinates gives the stated recurrence and coefficient formulas.
\end{proof}

At this point the matrix
\[
  \mathbf{S}^{-1}\mathbf{S}^{-\top}
\]
appears everywhere. It is therefore natural to encode the change of variables
through the SPD matrix
\[
  \mathbf{M}:=\mathbf{S}^{\top}\mathbf{S},
  \qquad
  \mathbf{M}^{-1}=\mathbf{S}^{-1}\mathbf{S}^{-\top}.
\]
This matrix is the \emph{preconditioner}. 
The ideal case would be $\mathbf{M}= \mathbf{S}^{\top}\mathbf{S}=\mathbf{A}$
, because then
\[
  \tilde{\mathbf{A}}
  =\mathbf{S}^{-\top}\mathbf{A}\mathbf{S}^{-1}
  =\mathbf{I}.
\]
In that ideal situation, CG would solve the transformed problem in one step in
exact arithmetic. Of course, such a preconditioner is usually as hard to build
as solving the original system, so in practice one looks for a matrix that only
approximates $\mathbf{A}$ but is cheap to invert or apply (e.g. diagonal).

Final prseudocode for preconditioned CG can be viewed in \cref{alg:ch5-pcg}. The terms
colored in \textcolor{crimson}{crimson} are precisely the replacements that
turn \cref{alg:ch5-linear-cg} into its preconditioned version.

\begin{algorithm}[ht]
  \caption{Linear CG and preconditioned CG. In the right column, the terms
  colored in \textcolor{crimson}{crimson} are the only changes relative to the
  linear method.}
  \label{alg:ch5-pcg}
  \centering
  \begin{minipage}[t]{0.48\linewidth}
    \hrule
    \vspace{3pt}
    {\bfseries \textsc{Linear CG}}\par
    \vspace{3pt}
    \hrule
    \vspace{3pt}
    \small
    \begin{algorithmic}
      \REQUIRE SPD matrix $\mathbf{A}$, vector $\mathbf{b}$, initial guess $\mathbf{x}_0$, tolerance $\varepsilon>0$
      \STATE $\mathbf{g}_0\gets \mathbf{A}\mathbf{x}_0-\mathbf{b}$, \ $\mathbf{d}_0\gets -\mathbf{g}_0$, \ $k\gets 0$
      \WHILE{$\|\mathbf{g}_k\|>\varepsilon \max\{1,\|\mathbf{b}\|\}$}
        \STATE $\alpha_k\gets \dfrac{\mathbf{g}_k^{\top}\mathbf{g}_k}{\mathbf{d}_k^{\top}\mathbf{A}\mathbf{d}_k}$
        \STATE $\mathbf{x}_{k+1}\gets \mathbf{x}_k+\alpha_k \mathbf{d}_k$
  ...
  \begin{minipage}[t]{0.48\linewidth}
    \hrule
    \vspace{3pt}
    {\bfseries \textsc{Preconditioned CG}}\par
    \vspace{3pt}
    \hrule
    \vspace{3pt}
    \small
    \begin{algorithmic}
      \REQUIRE SPD matrix $\mathbf{A}$, vector $\mathbf{b}$, SPD preconditioner $\mathbf{M}$, initial guess $\mathbf{x}_0$, tolerance $\varepsilon>0$
      \STATE $\mathbf{g}_0\gets \mathbf{A}\mathbf{x}_0-\mathbf{b}$, \ $\mathbf{d}_0\gets -\textcolor{crimson}{\mathbf{M}^{-1}\mathbf{g}_0}$, \ $k\gets 0$
      \WHILE{$\|\mathbf{g}_k\|>\varepsilon \max\{1,\|\mathbf{b}\|\}$}
        \STATE $\alpha_k\gets \dfrac{\textcolor{crimson}{\mathbf{g}_k^{\top}\mathbf{M}^{-1}\mathbf{g}_k}}{\mathbf{d}_k^{\top}\mathbf{A}\mathbf{d}_k}$
        \STATE $\mathbf{x}_{k+1}\gets \mathbf{x}_k+\alpha_k \mathbf{d}_k$
        \STATE $\mathbf{g}_{k+1}\gets \mathbf{g}_k+\alpha_k \mathbf{A}\mathbf{d}_k$
        \STATE $\beta_k\gets \dfrac{\textcolor{crimson}{\mathbf{g}_{k+1}^{\top}\mathbf{M}^{-1}\mathbf{g}_{k+1}}}{\textcolor{crimson}{\mathbf{g}_k^{\top}\mathbf{M}^{-1}\mathbf{g}_k}}$
        \STATE $\mathbf{d}_{k+1}\gets -\textcolor{crimson}{\mathbf{M}^{-1}\mathbf{g}_{k+1}}+\beta_k \mathbf{d}_k$
        \STATE $k\gets k+1$
      \ENDWHILE
      \RETURN $\mathbf{x}_k$
    \end{algorithmic}
...
\begin{example}[Diagonal preconditioning (see \cref{fig:ch5-pcg-example})]
Consider
\[
  \mathbf{A}\mathbf{x}=\mathbf{b},
  \qquad
...
\[
  ...
  \end{cases}
  \qquad
  \mathbf{M}=\operatorname{Diag}(\mathbf{A}) \approx \mathbf{A}.
\]
...
\Cref{alg:ch5-nonlinear-cg} places linear CG and Fletcher--Reeves side by
side. The terms colored in \textcolor{crimson}{crimson} are exactly the parts
that change when we pass from the quadratic model to a general smooth
objective.

\begin{algorithm}[ht]
  \caption{Linear CG and Fletcher--Reeves nonlinear CG. In the right column,
  the terms colored in \textcolor{crimson}{crimson} are the only changes
  relative to the linear method.}
  ...
  \begin{minipage}[t]{0.48\linewidth}
    \hrule
    \vspace{3pt}
    {\bfseries \textsc{Fletcher--Reeves}}\par
    \vspace{3pt}
    \hrule
    \vspace{3pt}
    \small
    \begin{algorithmic}
      \REQUIRE \textcolor{crimson}{$f$, $\nabla f$}, initial point $\mathbf{x}_0$, tolerance $\varepsilon>0$
      \STATE $\mathbf{g}_0\gets \textcolor{crimson}{\nabla f(\mathbf{x}_0)}$, \ $\mathbf{d}_0\gets -\mathbf{g}_0$, \ $k\gets 0$
      \WHILE{$\|\mathbf{g}_k\|>\textcolor{crimson}{\varepsilon \|\nabla f(\mathbf{x}_0)\|}$}
        \STATE $\alpha_k\gets \textcolor{crimson}{\text{line search}}$
        \STATE $\mathbf{x}_{k+1}\gets \mathbf{x}_k+\alpha_k \mathbf{d}_k$
        \STATE $\mathbf{g}_{k+1}\gets \textcolor{crimson}{\nabla f(\mathbf{x}_{k+1})}$
        \STATE $\beta_k\gets \dfrac{\mathbf{g}_{k+1}^{\top}\mathbf{g}_{k+1}}{\mathbf{g}_k^{\top}\mathbf{g}_k}$
        \STATE $\mathbf{d}_{k+1}\gets -\mathbf{g}_{k+1}+\beta_k \mathbf{d}_k$
        \STATE $k\gets k+1$
      \ENDWHILE
...
generated direction is still a descent direction. 

\subsection{Other \texorpdfstring{$\beta_k$}{beta} formulas}
Different nonlinear-CG schemes differ in how the coefficient $\beta_k$ is
chosen. Let
\[
...
\end{array}
\]
Out of this set the most popural option is Polak--Ribiere.
...
away the accumulated direction history and restart from gradient descent. 

\begin{example}[Two standard test problems (see \cref{fig:ch5-nonlinear-cg-comparison})]
...
The memory footprint of nonlinear CG is small and equals $O(n)$. The cost of one
iteration is $O(n)$. In
problems where gradient descent and nonlinear CG use the same line-search
accuracy, nonlinear CG is usually faster in practice because the search
direction reuses curvature information from previous steps instead of restarting
from the steepest-descent direction every iteration. So, CG and GD have the same memory
and computational cost, but CG converges, usually, much faster.

\begin{figure}[ht]
  \centering
  \incfig{nonlinear-cg-comparison-embed}
